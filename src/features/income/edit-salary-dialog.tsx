...
interface SalaryData {
  ...
  netPay: string;
  payDetail: Record<string, unknown>;
  adjustmentDetail: Record<string, unknown>;
...
function toKVPairs(obj: Record<string, unknown>): KVPair[] {
...
export function EditSalaryDialog({ salary, open, onClose, onSaved }: EditSalaryDialogProps) {
  ...
  const [updateSalary, { loading }] = useUpdateSalaryMutation();

  useEffect(() => {
    if (salary) {
      setDate(salary.date);
      setGrossPay(salary.grossPay);
      setTotalAdjustment(salary.totalAdjustment);
      setTotalWithheld(salary.totalWithheld);
      setTotalDeduction(salary.totalDeduction);
      setNetPay(salary.netPay);
      setPayPairs(toKVPairs(salary.payDetail));
      setAdjustmentPairs(toKVPairs(salary.adjustmentDetail));
      setTaxPairs(toKVPairs(salary.taxDetail));
      setDeductionPairs(toKVPairs(salary.deductionDetail));
    }
  }, [salary]);
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Salary — {date}</DialogTitle>
        </DialogHeader>
